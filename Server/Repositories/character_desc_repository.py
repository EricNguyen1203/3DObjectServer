...
from Models.character_desc_enity import CharacterDescriptionEntity
class CharacterDescriptionRepository(BaseRepository):
    
    def __init__(self):
        super().__init__()  
        self._collection_name = "character_description"
        self._collection_instance = self._mongo_instance.get_collection(collection_name=self._collection_name)
    
    def insert_one(self, data: CharacterDescriptionEntity):
        result = self._collection_instance.insert_one(data.to_dict())
        print("Inserted ID:", result.inserted_id)
        return result.inserted_id
    
    def insert_many(self, data: List[CharacterDescriptionEntity]):
